...
qboolean CG_IsEnemy(const clientInfo_t *target) {
  ...
  if (myStateTeam == TEAM_SPECTATOR) {
    ...
    if (cg_spectPOV.integer && cg.snap->ps.pm_flags & PMF_FOLLOW &&
      ...
      if (cg_debugAnim.integer) {
        CG_Printf("[CG_IsEnemy] Following %d (team %d) | Target: %s (team %d) "
                  "| isEnemy=%d\n",
                  cg.snap->ps.clientNum, ourPerspectiveTeam, target->name,
                  targetTeam, result);
      }
...
char *cg_customSoundNames[MAX_CUSTOM_SOUNDS] = {
    ...
    "*falling1.wav", "*gasp.wav",     "*drown.wav",    "*fall1.wav",
    "*taunt.wav"};
...
sfxHandle_t CG_CustomSound(int clientNum, const char *soundName) {
  ...
  for (i = 0; i < MAX_CUSTOM_SOUNDS && cg_customSoundNames[i]; i++) {
    if (!strcmp(soundName, cg_customSoundNames[i])) {
      ...
      } else if ((myteam != TEAM_FREE && ci->team == myteam && ci != myself) &&
                 cgs.teamSounds[i]) {
        return cgs.teamSounds[i];
      } else if (((ci->team != myteam) ||
                  (myteam == TEAM_FREE && ci != myself)) &&
                 cgs.enemySounds[i]) {
...
static qboolean CG_RegisterClientSkin(clientInfo_t *ci, const char *teamName,
  ...
  Com_sprintf(filename, MAX_QPATH, "models/players/%s/%slower_%s.skin",
  ...
  Com_sprintf(filename, MAX_QPATH, "models/players/%s/%supper_%s.skin",
              modelName, teamName, skinName);
  ...
  if (headModelName[0] == '*') {
    Com_sprintf(filename, MAX_QPATH, "models/players/heads/%s/head_%s.skin",
                &headModelName[1], headSkinName);
  } else {
    Com_sprintf(filename, MAX_QPATH, "models/players/%s/%shead_%s.skin",
                headModelName, teamName, headSkinName);
  }
...
CG_RegisterClientModelname(clientInfo_t *ci, const char *modelName,
  ...
  if (!ci->legsModel) {
    Com_sprintf(filename, sizeof(filename),
                "models/players/characters/%s/lower.md3", modelName);
    ci->legsModel = trap_R_RegisterModel(filename);
    if (!ci->legsModel) {
  ...
  if (!ci->torsoModel) {
    Com_sprintf(filename, sizeof(filename),
                "models/players/characters/%s/upper.md3", modelName);
    ci->torsoModel = trap_R_RegisterModel(filename);
    if (!ci->torsoModel) {
      Com_Printf("Failed to load model file %s\n", filename);
      return qfalse;
  ...
  if (headName[0] == '*') {
    Com_sprintf(filename, sizeof(filename), "models/players/heads/%s/%s.md3",
                &headModelName[1], &headModelName[1]);
  } else {
    Com_sprintf(filename, sizeof(filename), "models/players/%s/head.md3",
                headName);
  }
  ci->headModel = trap_R_RegisterModel(filename);
  // if the head model could not be found and we didn't load from the heads
  // folder try to load from there
  ...
  if (!CG_RegisterClientSkin(ci, teamName, modelName, skinName, headName,
                             headSkinName)) {
    failed_flag = qtrue;
  ...
  if (headName[0] == '*') {
    Com_sprintf(filename, sizeof(filename),
                "models/players/heads/%s/icon_%s.tga", &headName[1],
                headSkinName);
  } else {
    Com_sprintf(filename, sizeof(filename), "models/players/%s/icon_%s.tga",
  ...
  if (!ci->modelIcon) {
    if (headName[0] != '*') {
      // here
      Com_sprintf(filename, sizeof(filename),
                  "models/players/heads/%s/icon_%s.tga", headName,
                  headSkinName);
      ci->modelIcon = trap_R_RegisterShaderNoMip(filename);
    }
  }
...
static void CG_RegisterOriginalHeadModel(clientInfo_t *ci) {
  ...
  if (headModelName[0] == '*') {
    Com_sprintf(filename, sizeof(filename),
                "models/players/heads/%s/head_%s.skin", &headModelName[1],
                headSkinName);
  } else {
    Com_sprintf(filename, sizeof(filename), "models/players/%s/head_%s.skin",
                headModelName, headSkinName);
  }

  ci->originalHeadSkinHandle = trap_R_RegisterSkin(filename);

  /* if the head skin could not be found and we didn't load from the heads
   * folder try to load from there */
  if (!ci->originalHeadSkinHandle && headModelName[0] != '*') {
    Com_sprintf(filename, sizeof(filename),
                "models/players/heads/%s/head_%s.skin", headModelName,
                headSkinName);
    ci->originalHeadSkinHandle = trap_R_RegisterSkin(filename);
  }
  ...
  if (headModelName[0] == '*') {
    Com_sprintf(filename, sizeof(filename),
                "models/players/heads/%s/icon_%s.tga", &headModelName[1],
                headSkinName);
  } else {
    Com_sprintf(filename, sizeof(filename), "models/players/%s/icon_%s.tga",
                headModelName, headSkinName);
  ...
  /* if the head icon could not be found and we didn't load from the heads
   * folder try to load from there */
  if (!ci->originalModelIcon && headModelName[0] != '*') {
    Com_sprintf(filename, sizeof(filename),
                "models/players/heads/%s/icon_%s.tga", headModelName,
                headSkinName);
    ci->originalModelIcon = trap_R_RegisterShaderNoMip(filename);
  }
}
...
void CG_LoadForcedSounds(void) {
  ...
  trap_Cvar_VariableStringBuffer("cg_mySound", mySoundModel,
                                 sizeof(mySoundModel));
  trap_Cvar_VariableStringBuffer("cg_teamSound", teamSoundModel,
                                 sizeof(teamSoundModel));
  trap_Cvar_VariableStringBuffer("cg_enemySound", enemySoundModel,
                                 sizeof(enemySoundModel));
  ...
  for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
    ...
    if (mySoundModel[0]) {
      cgs.mySounds[i] = trap_S_RegisterSound(
          va("sound/player/%s/%s", mySoundModel, s + 1), qfalse);
    }
    if (teamSoundModel[0]) {
      cgs.teamSounds[i] = trap_S_RegisterSound(
          va("sound/player/%s/%s", teamSoundModel, s + 1), qfalse);
    }
    if (enemySoundModel[0]) {
      cgs.enemySounds[i] = trap_S_RegisterSound(
          va("sound/player/%s/%s", enemySoundModel, s + 1), qfalse);
    }
  }
}
...
static void CG_LoadClientInfo(clientInfo_t *ci) {
  ...
  if (!CG_RegisterClientModelname(ci, ci->modelName, ci->skinName,
    ...
    if (cg_buildScript.integer) {
      CG_Error("CG_RegisterClientModelname( %s, %s, %s, %s %s ) failed",
  ...
  for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
    ...
    if (modelloaded) {
      ci->sounds[i] =
          trap_S_RegisterSound(va("sound/player/%s/%s", dir, s + 1), qfalse);
    }
    if (!ci->sounds[i]) {
      ci->sounds[i] = trap_S_RegisterSound(
          va("sound/player/%s/%s", fallback, s + 1), qfalse);
    }
  }
...
static void CG_UpdateModelFromString(char *modelName, char *skinName,
                                     const char *resultModelString,
                                     qboolean isOurClient, team_t team) {
...
static void CG_ClientInfoUpdateModel(clientInfo_t *ci, qboolean isOurClient,
                                     qboolean isTeamGame, const char *config,
                                     int clientNum) {
  ...
  if (isOurClient) {
    ...
    trap_Cvar_VariableStringBuffer("headmodel", headModelName,
                                   sizeof(headModelName));
    resultHModelString = headModelName;
  } else {
    const char *forceModelString =
        cg_forceModel.integer ? cfgModelString : NULL;
    const char *forceHModelString =
        cg_forceModel.integer ? cfgHModelString : NULL;
    const char *enemyModelString = NULL;
    const qboolean useOriginal = cg_spectOrigModel.integer &&
                                 CG_IsFollowing() &&
    ...
    if (!isTeamGame) {
      ...
      if (cg_spectPOV.integer &&
          ...
          cg.snap->ps.pm_flags & PMF_FOLLOW && cg.snap->ps.clientNum >= 0 &&
      ...
      const char *teamModelString =
          cg_teamModel.string[0] ? cg_teamModel.string : NULL;
      qboolean isTeamMate;
      ...
      if (cg_spectPOV.integer) {
        ...
        if (cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR) {
          if (cg.snap->ps.pm_flags & PMF_FOLLOW && cg.snap->ps.clientNum >= 0 &&
              cg.snap->ps.clientNum < MAX_CLIENTS) {
            ourPerspectiveTeam = cgs.clientinfo[cg.snap->ps.clientNum].rt;
          } else {
  ...
  CG_UpdateModelFromString(&ci->modelName[0], &ci->skinName[0],
                           resultModelString, isOurClient, ci->rt);
  CG_UpdateModelFromString(&ci->headModelName[0], &ci->headSkinName[0],
                           resultHModelString, isOurClient, ci->rt);
...
#define NUM_TO_HEX_CHAR(III)                                                   \
  (((III & 0x0f) < 10) ? ('0' + (III & 0x0f)) : ('a' + ((III & 0x0f) - 10)))
...
void CG_NewClientInfo(int clientNum) {
  ...
  CG_StringMakeEscapeCharRAW(newInfo.name_original, newInfo.name_codes,
                             sizeof(newInfo.name_codes));
  ...
  if (cg_playersID.integer == 1) {
      ...
      if (cg_playersXID.integer == 2) {
        ...
        for (p = 0; p < MAX_CLIENTS; ++p) {
          ...
          if (cgs.clientinfo[p].infoValid &&
              !cgs.clientinfo[p].nameIsInvisible &&
              strcmp(newInfo.name_clean, cgs.clientinfo[p].name_clean) == 0) {
            is_exists = qtrue;
            // okay, it exits. check is another client have xid in the name and
            // append it if not
            if (strcmp(cgs.clientinfo[p].name_original,
                       cgs.clientinfo[p].name) == 0) {
              Com_sprintf(cgs.clientinfo[p].name, sizeof(ci->name),
                          "[%d]^8%s^7:%s", p, cgs.clientinfo[p].xidStr,
                          cgs.clientinfo[p].name_original);
            }
            break;
          }
        }
        ...
        if (newInfo.nameIsInvisible) {
          Com_sprintf(newInfo.name, sizeof(newInfo.name), "[%d]^8%s^7:\'%s^7\'",
                      clientNum, newInfo.xidStr, newInfo.name_codes);
        } else if (is_exists ||
                   (strcmp(newInfo.name_original, "^1NONAME") == 0)) {
          Com_sprintf(newInfo.name, sizeof(newInfo.name), "[%d]^8%s^7:%s",
                      clientNum, newInfo.xidStr, newInfo.name_original);
        } else {
          Com_sprintf(newInfo.name, sizeof(newInfo.name), "[%d]%s", clientNum,
                      newInfo.name_original);
        }
      } else {
        Com_sprintf(newInfo.name, sizeof(newInfo.name), "[%d]^8%s^7:%s",
                    clientNum, newInfo.xidStr, newInfo.name_original);
    ...
    if (cg_playersXID.integer) {
      ...
      for (p = 0; p < MAX_CLIENTS; ++p) {
        ...
        // ugly linear search
        if (cgs.clientinfo[p].infoValid && !cgs.clientinfo[p].nameIsInvisible &&
            strcmp(newInfo.name_clean, cgs.clientinfo[p].name_clean) == 0) {
          is_exists = qtrue;
          // okay, it exits. check is another client have id in the name and
          // append it if not
          if (strcmp(cgs.clientinfo[p].name_original, cgs.clientinfo[p].name) ==
              0) {
            Com_sprintf(cgs.clientinfo[p].name, sizeof(ci->name), "[%d]%s", p,
                        cgs.clientinfo[p].name_original);
          }
          break;
        }
    ...
    if (cg_playersXID.integer == 2) {
      ...
      for (p = 0; p < MAX_CLIENTS; ++p) {
        ...
        if (cgs.clientinfo[p].infoValid && !cgs.clientinfo[p].nameIsInvisible &&
          ...
          if (strcmp(cgs.clientinfo[p].name_original, cgs.clientinfo[p].name) ==
              0) {
            Com_sprintf(cgs.clientinfo[p].name, sizeof(ci->name), "^8%s^7:%s",
                        cgs.clientinfo[p].xidStr,
                        cgs.clientinfo[p].name_original);
          }
          break;
        }
      ...
      if (newInfo.nameIsInvisible) {
        Com_sprintf(newInfo.name, sizeof(newInfo.name), "^8%s^7:\'%s^7\'",
                    newInfo.xidStr, newInfo.name_codes);
      } else if (is_exists ||
                 (strcmp(newInfo.name_original, "^1NONAME") == 0)) {
        Com_sprintf(newInfo.name, sizeof(newInfo.name), "^8%s^7:%s",
                    newInfo.xidStr, newInfo.name_original);
      } else {
        Q_strncpyz(newInfo.name, newInfo.name_original, sizeof(newInfo.name));
      }
    } else {
  ...
  CG_OSPColorsFromString(v, newInfo.colors.railCore, newInfo.colors.head,
                         newInfo.colors.torso, newInfo.colors.legs);
...
void CG_UpdateOurClientInfo(void) { CG_NewClientInfo(cg.clientNum); }
...
===============
*/
static void CG_RunLerpFrame(clientInfo_t *ci, lerpFrame_t *lf, int newAnimation,
                            float speedScale) {
  int f, numFrames;
  animation_t *anim;
  ...
  if (cg.time >= lf->frameTime) {
    ...
    if (anim->reversed) {
      ...
      lf->frame =
          anim->firstFrame + anim->numFrames - 1 - (f % anim->numFrames);
    } else {
...
static void CG_PlayerAnimation(centity_t *cent, int *legsOld, int *legs,
  ...
  *legsBackLerp = cent->pe.legs.backlerp;
  ...
  if (cg_noTaunt.integer &
      ((cent->currentState.torsoAnim & ~ANIM_TOGGLEBIT) == TORSO_GESTURE)) {
    cent->currentState.torsoAnim =
        (cent->currentState.torsoAnim & ANIM_TOGGLEBIT) | TORSO_STAND;
  }

  CG_RunLerpFrame(ci, &cent->pe.torso, cent->currentState.torsoAnim,
                  speedScale);
...
===============
*/
static void CG_PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t torso[3],
                            vec3_t head[3]) {
...
static void CG_HasteTrail(centity_t *cent) {
  ...
  smoke = CG_SmokePuff(origin, vec3_origin, 8, 1, 1, 1, 1, 500, cg.time, 0, 0,
...
static void CG_BreathPuffs(centity_t *cent, refEntity_t *head) {
  ...
  CG_SmokePuff(origin, up, 16, 1, 1, 1, 0.66f, 1500, cg.time, cg.time + 400,
...
static void CG_PlayerPowerups(centity_t *cent) {
  ...
  if (cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR) {
    ...
    if (cg_spectGlow.integer || clientNum == cg.snap->ps.generic1 - 32) {
      if (cgs.clientinfo[clientNum].team == TEAM_BLUE) {
        trap_R_AddLightToScene(cent->lerpOrigin, (float)((rand() & 0x1f) + 200),
                               0.2, 0.2, 1.0);
        trap_R_AddLightToScene(cent->lerpOrigin, (float)((rand() & 0x1f) + 200),
                               0.2, 0.2, 1.0);
        ...
        trap_R_AddLightToScene(cent->lerpOrigin,
                               (float)((rand() & 0x1f) + 0xc8), 1.0, 1.0, 1.0);
      }
    }
  }
  ...
  if (powerups & (1 << PW_QUAD)) {
    trap_R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 0.2f, 0.2f,
                           1);
  }
  ...
  if (powerups & (1 << PW_FLIGHT)) {
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           vec3_origin, cgs.media.flightSound);
  }

  // redflag
  if (powerups & (1 << PW_REDFLAG)) {
    CG_TrailItem(cent, cgs.media.redFlagModel);
    trap_R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 1.0, 0.2f,
                           0.2f);
  }
  ...
  if (powerups & (1 << PW_NEUTRALFLAG)) {
    ...
    trap_R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 1.0, 1.0,
                           1.0);
  }
...
static void CG_PlayerFloatSprite(centity_t *cent, qhandle_t shader,
  ...
  ent.renderfx = rf;
  if (color) {
    MAKERGBA(ent.shaderRGBA, color[0] * 255, color[1] * 255, color[2] * 255,
             255);
  } else {
...
static void CG_FriendHudMarker(centity_t *cent) {
  ...
  if (cgs.gametype < GT_TEAM || !cg_teamIndicator.integer ||
  ...
  if (!(cg_friendsWallhack.integer & 1) && !CG_FriendVisible(cent) ||
      !CG_BE_FEATURE_ENABLED(CG_BE_TEAM_FOE_WH) && !CG_FriendVisible(cent)) {
    return;
  }

  distance = Distance(cent->lerpOrigin, cg.predictedPlayerState.origin);
  if (cg_friendHudMarkerMaxDist.integer > 0 &&
      distance > cg_friendHudMarkerMaxDist.value) {
    return;
  }
  ...
  CG_HudBorderMarker(cent->lerpOrigin, color,
                     cg_friendHudMarkerSize.value * size,
                     CG_GetPlayerSpriteShader(cent), 270);
}
...
static void CG_PlayerSprites(centity_t *cent) {
  ...
  if (cg_drawFriend.integer) {
    if (!(cent->currentState.eFlags & EF_DEAD) &&
        cg.snap->ps.persistant[PERS_TEAM] == cl->team &&
        cgs.gametype >= GT_TEAM) {
      if (cg_teamFrozenFoe.integer && cgs.osp.gameTypeFreeze &&
          cent->currentState.powerups & (1 << PW_BATTLESUIT) &&
          cent->currentState.weapon == WP_NONE) {
        qhandle_t shader;
        if (CG_BE_FEATURE_ENABLED(CG_BE_TEAM_FOE_WH) &&
            cg_friendsWallhack.integer & 1)
...
static qboolean CG_PlayerShadow(centity_t *cent, float *shadowPlane) {
  ...
  *shadowPlane = trace.endpos[2] + 1;
  ...
  if (cg_shadows.integer != 1 &&
      cg_shadows.integer != -1) // no mark for stencil or projection shadows
  {
    return qtrue;
  }
  ...
  CG_ImpactMark(shadowMarkShader, trace.endpos, trace.plane.normal, yawAngle,
                color[0], color[1], color[2], 1, qfalse, 24, qtrue, qtrue);
...
static void CG_PlayerSplash(centity_t *cent) {
  ...
  // this won't handle moving water brushes, but they wouldn't draw right
  // anyway...
...
void CG_AddRefEntityFrozenPlayer(refEntity_t *ent, entityState_t *state,
                                 int team) {
  const clientInfo_t *ci;

  if (state->clientNum >= MAX_CLIENTS)
    return;
...
void CG_AddHitBox(centity_t *cent, team_t team) {
  ...
  // don't draw it if it's us in first-person, for dead players, for frozen
  // players, for invisible players
  if (cent->currentState.number == cg.predictedPlayerState.clientNum &&
          !cg.renderingThirdPerson ||
      cent->currentState.eFlags & EF_DEAD ||
      (cgs.osp.gameTypeFreeze && cent->currentState.weapon == WP_NONE &&
       cent->currentState.powerups & (1 << PW_BATTLESUIT)) ||
...
void CG_AddOutline(refEntity_t *ent, centity_t *cent) {
  ...
  ent->customShader =
      isEnemy ? cgs.media.outlineShader : cgs.media.teamOutlineShader;
  ...
  if (isSpectator && !cg_spectPOV.integer) {
    ...
    if (CG_BE_FEATURE_ENABLED(CG_BE_MARK_TEAMMATE) &&
...
void CG_Player(centity_t *cent) {
  ...
  //
  paintItBlack =
      cg_deadBodyBlack.integer &&
      (cent->currentState.eFlags & EF_DEAD || // dead
       (cgs.osp.gameTypeFreeze && (cent->currentState.weapon == WP_NONE) &&
        (cent->currentState.powerups & (1 << PW_BATTLESUIT))) // or frozen
      );
  if (paintItBlack) {
  ...
  if ((cent->currentState.eFlags & EF_DEAD) && cg_deadBodyInvisible.integer) {
    if (cent->currentState.weapon ==
        WP_NONE) // show player model with weapon during intermission vote -
  ...
  if (ci->isPmSkin ||
      (ci->isFbSkin && CG_BE_FEATURE_ENABLED(CG_BE_FULLBRIGHT))) {
    float tmpf;
  ...
  if (paintItBlack) {
    torso.shaderRGBA[0] = torso.shaderRGBA[1] = torso.shaderRGBA[2] =
        paintBlackLevel;
  }
  ...
  if (paintItBlack) {
    head.shaderRGBA[0] = head.shaderRGBA[1] = head.shaderRGBA[2] =
        paintBlackLevel;
  }
