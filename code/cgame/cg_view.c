...
static int CG_CalcFov(void)
{
	...
	else
	{
		...
		if (zoomTime)
		{
			f = (cg.time - cg.zoomTime) / (float)zoomTime;
		}
		else
		{
			f = 1.0;
	...
	if (contents & (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA) && cg_underwaterFovWarp.integer)
	{
...
void CG_DrawActiveFrame(int serverTime, stereoFrame_t stereoView, qboolean demoPlayback)
{
	...
	// update cvars
	if (cgs.osp.cvarsUpdateDelay++ > 5)
	{
		CG_UpdateCvars();
		cgs.osp.cvarsUpdateDelay = 0;
	}
	...
	if (!cg.renderingThirdPerson && cg_damageDraw.integer)
	{
		CG_DamageBlendBlob();
	}
