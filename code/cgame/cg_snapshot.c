...
void CG_SetInitialSnapshot(snapshot_t* snap)
{
	...
	version = Info_ValueForKey(CG_ConfigString(0), "gameversion");
	if (version)
	{
		if ((Q_stricmp(version, "OSP v1") >= 0) && (strstr(version, "OSP Tourney") == 0))
		{
			cgs.osp.isOSPv1 = qtrue;
			CG_OSPUpdateUserInfo(qtrue);
		}
	}
	if (cgs.osp.numberOfStringsMotd && (!cg.demoPlayback || !cg_NoMOTDDisplayDuringDemo.integer))
	{
		CG_OSPMoTD_f();
	}
