...
=============
*/
void CG_OSPObituaryFreeze(entityState_t* es, char* targetName)
{
	char actorName[64];
	int actorId;
	int targetId;
	char actorNameNormalized[64];
	char targetNameNormalized[64];
	...
	cs = CG_ConfigString(actorId + CS_PLAYERS);
	if (!cs) return;

	Q_strncpyz(&actorName[0], Info_ValueForKey(cs, "n"), 64);
	if (rand() & 1)
	{
		actionStr = "thawed";
		endStr = "like a package of frozen peas";
	}
	else
	{
		actionStr = "evicted";
		switch (cgs.clientinfo[targetId].gender)
		{
			case GENDER_MALE:
				endStr = "from his igloo";
				break;
			case GENDER_FEMALE:
				endStr = "from her igloo";
				break;
			default:
			case GENDER_NEUTER:
				endStr = "from its igloo";
				break;
		}
	}
...
static void CG_Obituary(entityState_t* ent)
{
	...
	clientInfo_t*    ci;
	score_t* scores;
	qboolean isUnfreezeEvent = qfalse;
	...
	isUnfreezeEvent = (mod == MOD_UNKNOWN) && cgs.osp.gameTypeFreeze;
	...
	CG_RemoveChatEscapeChar(&targetName[0]);
	...
	else
	{
		...
		CG_RemoveChatEscapeChar(attackerName);
	}
	...
	if (attacker != ENTITYNUM_WORLD)
	{
		...
		if (cg.demoPlayback)
		{
			...
			for (i = 0; i < 64; ++i)
			{
				if (cg.scores[i].client == attacker)
				{
					scores = &cg.scores[i];
					break;
				}
			}
			if ((cgs.gametype == GT_TEAM) && (cgs.clientinfo[attacker].team == cgs.clientinfo[target].team))
			{
				--scores->score;
			}
			else if (target != cg.snap->ps.clientNum)
			{
				++scores->score;
			}
		}
...
//		CG_Printf( "EV_JUMP_PAD w/effect #%i\n", es->eventParm );
			{
				...
				(void)CG_SmokePuff(cent->lerpOrigin, up,
				                   ...
				                   cg.time, 0,
				                   LEF_PUFF_DONT_SCALE,
				                   (cg_nomip.integer & 0x100) ? cgs.media.smokePuffNoPicMipShader : cgs.media.smokePuffShader);
			}
